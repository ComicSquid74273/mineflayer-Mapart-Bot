...
| `advanced.supportStockXpBottleMinStacks` | `5` | Minimum XP bottle stacks expected across `machine.xpBottleChests`. | Configure all active XP bottle sources in `machine.xpBottleChests`. |
...
| `advanced.postPrintXpBottlePullStacks` | `1` | XP bottle stacks to pull from configured `machine.xpBottleChests` when below minimum XP. | Used before anvil rename; leftover bottles are returned by default. |
...
| `advanced.postPrintReturnUnusedXpBottles` | `true` | Return unused XP bottles to the first configured XP bottle chest after reaching the minimum level. | Set `false` only if you want leftovers kept in inventory. |
