...
function createStore(baseDir) {
  ...
  function isSlaveBot(botName) {
    return String(getBot(String(botName || '').trim())?.role || '').trim().toLowerCase() === 'slave'
  }
  ...
  function listNodesFromMap(botMapInput) {
    ...
    for (const bot of Object.values(botMap)) {
      ...
      const connectedHost = String(bot.connectedHost || '').trim()
      ...
      if (botOnline && connectedHost) {
        if (!current.connectedHosts.includes(connectedHost)) current.connectedHosts.push(connectedHost)
        if (botLastStatusMs >= current._connectedHostStatusAtMs) {
          current.connectedHost = connectedHost
          current._connectedHostStatusAtMs = botLastStatusMs
        }
      }
    ...
    for (const node of byHost.values()) {
      ...
      delete node._connectedHostStatusAtMs
      node.connectedHosts.sort((left, right) => String(left).localeCompare(String(right), undefined, { sensitivity: 'base' }))
  ...
  function reconcileQueueForBotLocalWork(status) {
    ...
    if (!botName || !hostLabel || String(status?.role || '').trim().toLowerCase() === 'slave') return
  ...
  function upsertBotStatus(status) {
    ...
    const slaveStatus = String(status.role || '').trim().toLowerCase() === 'slave'
    if (slaveStatus) {
      const inventory = readNodeInventoryMap()
      if (inventory[status.botName]) {
        delete inventory[status.botName]
        writeNodeInventoryMap(inventory)
      }
    } else if (hasInventoryPayload) {
      const inventory = readNodeInventoryMap()
  ...
  function createCommand(input) {
    ...
    if (input.commandType === 'home-platform' && input.targetBotName) {
      const existing = items.find((item) => (
        item.targetBotName === input.targetBotName
        && item.commandType === 'home-platform'
  ...
  function setNodePreferredHost(hostLabel, host) {
    ...
    if (!value || value.toLowerCase() === 'default') {
      delete state.preferredHosts[label]
    } else {
      state.preferredHosts[label] = { host: value, updatedAt: nowIso() }
    }
    saveControlState(state)
    return state.preferredHosts[label]?.host || null
  }

  function getNodePreferredHost(hostLabel) {
    const label = String(hostLabel || '').trim()
    if (!label) return null
    const item = readControlState().preferredHosts[label]
    const host = item && typeof item === 'object' ? String(item.host || '').trim() : ''
    return host || null
  }
  ...
  function getNextAssignedFile(botName) {
    if (isSlaveBot(botName)) return null
  ...
  function claimNextNodeFile(hostLabel, botName) {
    const normalizedHost = String(hostLabel || '').trim()
    const normalizedBot = String(botName || '').trim()
    if (!normalizedHost || !normalizedBot || isSlaveBot(normalizedBot)) return null
    const items = listFiles()
  ...
  function claimNextQueueFiles(hostLabel, botName, limit = 1) {
    const normalizedHost = String(hostLabel || '').trim()
    const normalizedBot = String(botName || '').trim()
    if (!normalizedHost || !normalizedBot || isSlaveBot(normalizedBot)) return []
  ...
  function recoverActiveQueueFile(hostLabel, botName, details = {}) {
    const normalizedHost = String(hostLabel || '').trim()
    const normalizedBot = String(botName || '').trim()
    const localFileName = path.basename(String(details?.localFileName || details?.localPath || '').trim())
    const sha256 = String(details?.sha256 || '').trim().toLowerCase()
    if (!normalizedHost || !normalizedBot || !localFileName || !sha256 || isSlaveBot(normalizedBot)) return null

    const items = listFiles()
    const matches = items
      .map((item, index) => ({ item, index }))
      .filter(({ item }) => {
        if (!isQueueFile(item) || isTerminalQueueStatus(getQueueStatus(item))) return false
        if (String(item.claimedByBotName || '').trim() !== normalizedBot) return false
        if (String(item.claimedByHostLabel || '').trim() !== normalizedHost) return false
        if (String(item.sha256 || '').trim().toLowerCase() !== sha256) return false
        const names = [item.localFileName, item.originalName, item.fileName]
          .map((value) => path.basename(String(value || '').trim()))
          .filter(Boolean)
        return names.includes(localFileName)
      })
    if (matches.length !== 1) return null

    const match = matches[0]
    const current = match.item
    const currentStatus = getQueueStatus(current)
    const recoveredStatus = isActiveQueueStatus(currentStatus) ? currentStatus : 'held'
    items[match.index] = {
      ...current,
      claimedByBotName: normalizedBot,
      claimedByHostLabel: normalizedHost,
  ...
  // ---------- delivery bot (GetAllMaps) configuration ----------
  ...
  function isFreshBotStatus(bot, now = Date.now()) {
    const lastSeenMs = timestampMs(bot?.serverStatusAt || bot?.lastStatusAt || bot?.heartbeatAt)
    return Boolean(lastSeenMs && now - lastSeenMs >= 0 && now - lastSeenMs <= BOT_FRESH_MS)
  }

  function getDeliveryTargetReadiness(bot, now = Date.now()) {
    if (!bot || typeof bot !== 'object') {
      return {
        online: null,
        platformReady: false,
        readiness: 'unknown',
        readinessDetail: 'no live bot status'
      }
    }
    const fresh = isFreshBotStatus(bot, now)
    const online = fresh && bot.online === true
    const location = String(bot.locationDetail || bot.location || '').trim().toLowerCase()
    const platformReady = online && location === 'platform'
    return {
      online,
      platformReady,
      readiness: platformReady ? 'ready' : (online ? 'notready' : 'offline'),
      readinessDetail: platformReady
        ? 'online on platform'
        : (online ? `online at ${location || 'unknown'}` : 'offline or stale heartbeat')
    }
  }

  function defaultDeliveryConfig() {
    return {
      station: {
  ...
  function getDeliveryConfig() {
    const stored = readJson(deliveryConfigFile, null)
    const fallback = defaultDeliveryConfig()
    if (!stored || typeof stored !== 'object') return fallback
    return {
      ...fallback,
      ...stored,
      station: { ...fallback.station, ...(stored.station || {}) },
      manualTargets: Array.isArray(stored.manualTargets) ? stored.manualTargets : [],
      targetOverrides: stored.targetOverrides && typeof stored.targetOverrides === 'object' ? stored.targetOverrides : {}
    }
  }
  ...
  function updateDeliveryStation(patch) {
    const config = getDeliveryConfig()
    const station = { ...config.station }
    if (patch && typeof patch === 'object') {
      const anchor = sanitizePoint(patch.anchor)
      if (anchor) station.anchor = anchor
      const radius = Number(patch.radius)
      if (Number.isFinite(radius) && radius > 0) station.radius = radius
      if (typeof patch.homeName === 'string' && patch.homeName.trim()) station.homeName = patch.homeName.trim()
      for (const key of ['enderChest', 'dropChest', 'bundlesChest', 'openPosition']) {
        if (!Object.prototype.hasOwnProperty.call(patch, key)) continue
        station[key] = patch[key] == null ? null : (sanitizePoint(patch[key]) || station[key])
      }
    }
    config.station = station
    return saveDeliveryConfig(config).station
  }

  function upsertDeliveryTarget(entry) {
    const botName = String(entry?.botName || '').trim()
    if (!botName) return null
    const config = getDeliveryConfig()
    const bots = readBotMap()
    const isKnownBot = Boolean(bots[botName])

    if (isKnownBot) {
      const previous = config.targetOverrides[botName] || {}
      config.targetOverrides[botName] = {
        ...previous,
        enabled: entry.enabled !== false,
        order: Number.isFinite(Number(entry.order)) ? Number(entry.order) : toNumber(previous.order, 100),
        anchorOverride: Object.prototype.hasOwnProperty.call(entry, 'anchor') && entry.anchor != null
          ? sanitizePoint(entry.anchor)
          : (previous.anchorOverride || null),
        note: typeof entry.note === 'string' ? entry.note : previous.note || null,
        updatedAt: nowIso()
      }
    } else {
      const anchor = sanitizePoint(entry.anchor)
      if (!anchor) return null
      const manualTargets = config.manualTargets.filter((item) => String(item?.botName || '') !== botName)
      manualTargets.push({
        botName,
        anchor,
        enabled: entry.enabled !== false,
      ...
      config.manualTargets = manualTargets
    }
    saveDeliveryConfig(config)
    return listDeliveryTargets().find((item) => item.botName === botName) || null
  }

  function deleteDeliveryTarget(botName) {
    const safeName = String(botName || '').trim()
    if (!safeName) return false
    const config = getDeliveryConfig()
    const beforeManual = config.manualTargets.length
    config.manualTargets = config.manualTargets.filter((item) => String(item?.botName || '') !== safeName)
    const hadOverride = Object.prototype.hasOwnProperty.call(config.targetOverrides, safeName)
    if (hadOverride) delete config.targetOverrides[safeName]
    if (beforeManual === config.manualTargets.length && !hadOverride) return false
    saveDeliveryConfig(config)
    return true
  }

  // Merges auto-reported printer platform anchors with manual entries/overrides.
  function listDeliveryTargets() {
    const config = getDeliveryConfig()
    ...
    for (const bot of Object.values(bots)) {
      const botName = String(bot?.botName || '').trim()
      if (!botName) continue
      if (String(bot?.runtime || '') !== 'nerv-printer') continue
      if (String(bot?.role || '').toLowerCase() === 'delivery') continue
      const override = overrides[botName] || {}
      const anchor = sanitizePoint(override.anchorOverride) || sanitizePoint(bot.platformAnchor)
      if (!anchor) continue
      const readiness = getDeliveryTargetReadiness(bot, now)
      seen.add(botName)
      targets.push({
        botName,
        anchor,
        ...
        order: toNumber(override.order, 100),
        note: override.note || null,
        anchorOverridden: Boolean(sanitizePoint(override.anchorOverride)),
        online: readiness.online,
        platformReady: readiness.platformReady,
        ...
        hostLabel: bot.hostLabel || null,
        lastStatusAt: bot.lastStatusAt || null
      })
    }

    for (const manual of config.manualTargets) {
      const botName = String(manual?.botName || '').trim()
      if (!botName || seen.has(botName)) continue
      const anchor = sanitizePoint(manual.anchor)
      if (!anchor) continue
      targets.push({
        botName,
        anchor,
        source: 'manual',
        enabled: manual.enabled !== false,
        order: toNumber(manual.order, 100),
        note: manual.note || null,
        anchorOverridden: false,
        platformReady: false,
        readiness: 'unknown',
        readinessDetail: 'manual target has no live bot status',
        online: null,
        hostLabel: null,
        lastStatusAt: manual.updatedAt || null
      })
    }

    return targets.sort((left, right) => (left.order - right.order) || String(left.botName).localeCompare(String(right.botName)))
  }
