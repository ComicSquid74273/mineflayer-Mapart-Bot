...
const elements = {
  ...
  homeAllButton: document.getElementById('homeAllButton'),
  ...
  teleportWhitelistPanel: document.getElementById('teleportWhitelistPanel'),
  teleportWhitelistForm: document.getElementById('teleportWhitelistForm'),
  ...
  deliveryStationForm: document.getElementById('deliveryStationForm'),
  deliveryStationX: document.getElementById('deliveryStationX'),
  deliveryStationY: document.getElementById('deliveryStationY'),
  deliveryStationZ: document.getElementById('deliveryStationZ'),
  deliveryLocationSummary: document.getElementById('deliveryLocationSummary'),
  deliveryChatPanel: document.getElementById('deliveryChatPanel'),
  deliveryStationStatus: document.getElementById('deliveryStationStatus'),
  deliveryTargetsList: document.getElementById('deliveryTargetsList'),
  deliveryManualForm: document.getElementById('deliveryManualForm'),
  deliveryManualBotName: document.getElementById('deliveryManualBotName'),
  deliveryManualX: document.getElementById('deliveryManualX'),
  deliveryManualY: document.getElementById('deliveryManualY'),
  deliveryManualZ: document.getElementById('deliveryManualZ'),
  deliveryManualOrder: document.getElementById('deliveryManualOrder')
}
...
function displayHostLabel(hostLabel) {
  const text = String(hostLabel || '').trim()
  if (!text) return ''
  ...
  if (fleetHosts.size !== 1 || !fleetHosts.has(text)) return text

  const timestampedVm = text.match(/^(.*?)-\d{8}-\d{6}-(vm)$/i)
  return timestampedVm ? `${timestampedVm[1]}-${timestampedVm[2]}` : text
}

function isDeliveryBot(bot) {
  return String(bot?.role || '').toLowerCase() === 'delivery'
}

function fleetBots() {
  return state.bots.filter((bot) => !isDeliveryBot(bot))
}

function latestBotStatusAt(bots) {
  const timestamps = bots
    .map((bot) => bot.serverStatusAt || bot.lastStatusAt || bot.heartbeatAt || '')
    .filter(Boolean)
    .sort()
...
function fleetNodes() {
  const botsByHost = new Map()
  for (const bot of fleetBots()) {
    const hostLabel = String(bot.hostLabel || '').trim()
    if (!hostLabel) continue
    if (!botsByHost.has(hostLabel)) botsByHost.set(hostLabel, [])
    botsByHost.get(hostLabel).push(bot)
  }

  return (Array.isArray(state.nodes) ? state.nodes : [])
    .map((node) => {
      const hostLabel = String(node?.hostLabel || '').trim()
      const nodeBots = botsByHost.get(hostLabel) || []
      if (!hostLabel || !nodeBots.length) return null
      return {
        ...node,
        botNames: nodeBots.map((bot) => bot.botName).filter(Boolean),
        botCount: nodeBots.length,
        onlineCount: nodeBots.filter((bot) => bot.online === true).length,
        lastStatusAt: latestBotStatusAt(nodeBots) || node.lastStatusAt
      }
    })
    .filter(Boolean)
}
...
function renderFleetJump() {
  ...
  const knownNodes = fleetNodes().sort(compareNodeLabels)
  const visibleBots = fleetBots()
  ...
  elements.fleetJump.innerHTML = `
    <div class="fleet-jump-head">
      <span>Known nodes &middot; ${escapeHtml(visibleBots.length)} bots</span>
    </div>
    <div class="fleet-jump-buttons">
      ${knownNodes.map((node, index) => {
        ...
        const nodeBots = visibleBots.filter((bot) => String(bot.hostLabel || '').trim() === hostLabel)
        ...
        const displayLabel = displayHostLabel(hostLabel)
        const shortcutLabel = knownNodes.length === 1 ? displayLabel : nodeShortcutLabel(hostLabel, index)
        ...
        return `
          <button class="fleet-jump-button ${onlineClass}" type="button" data-action="jump-node" data-node-target="${escapeHtml(nodeAnchorId(hostLabel, index))}" title="${escapeHtml(`${displayLabel}: ${onlineCount}/${botCount} online, ${displayStatusText}`)}" aria-label="${escapeHtml(`Jump to ${displayLabel}`)}">
...
function renderSummary() {
  const visibleBots = fleetBots()
  const visibleNodes = fleetNodes()
  const reconnects = visibleBots.reduce((sum, item) => sum + Math.max(0, Number(item.reconnectCount || 0)), 0)
  const summarySignature = JSON.stringify({
    botCount: visibleBots.length,
    nodeCount: visibleNodes.length,
    online: visibleBots.filter((item) => item.online).length,
    printing: visibleBots.filter((item) => isBotPrinting(item)).length,
    paused: visibleBots.filter((item) => isBotPaused(item)).length,
    stale: visibleBots.filter((item) => item.activeState === 'stale').length,
    idle: visibleBots.filter((item) => item.idle && !isBotPaused(item)).length,
    reconnects
  })
  if (state.renderCache.summary === summarySignature) return
  state.renderCache.summary = summarySignature

  const online = visibleBots.filter((item) => item.online).length
  const printing = visibleBots.filter((item) => isBotPrinting(item)).length
  const paused = visibleBots.filter((item) => isBotPaused(item)).length
  const stale = visibleBots.filter((item) => item.activeState === 'stale').length
  const idle = visibleBots.filter((item) => item.idle && !isBotPaused(item)).length
  const nodeCount = visibleNodes.length

  elements.botSummary.innerHTML = [
    ['Known Nodes', nodeCount],
    ['Known Bots', visibleBots.length],
    ['Online', online],
    ['Printing', printing],
    ['Paused', paused],
...
function formatAlertNodeBotDetails(alert) {
  ...
  const addEntry = (hostLabel, botName, sinceAt = null) => {
    const host = displayHostLabel(hostLabel)
  ...
  if (!entries.length && Array.isArray(details.hostLabels)) {
    const hosts = details.hostLabels.map(displayHostLabel).filter(Boolean)
...
function renderResourceMetrics() {
  ...
  const fleetHostLabels = new Set(fleetNodes().map((node) => String(node.hostLabel || '').trim()).filter(Boolean))
  const nodes = (Array.isArray(state.resourceMetrics.nodes) ? state.resourceMetrics.nodes : [])
    .filter((node) => fleetHostLabels.has(String(node?.hostLabel || '').trim()))
  ...
  const nodeRows = nodes.map((node) => {
    ...
    return buildResourceMetricRow(displayHostLabel(node.hostLabel) || 'unknown-node', node.runtimeMetrics, statusText, node.latencyMs)
  }).join('')
  elements.resourceMetricsGrid.innerHTML = `
...
function deliveryExpectedUsernames(processState = {}) {
  const connection = String(processState?.connection || processState?.detectedConnection || processState?.activeConnection || '').trim()
  const profileUsernames = connection && processState?.connectionUsernames
    ? processState.connectionUsernames[connection]
    : null
  const configured = Array.isArray(profileUsernames) && profileUsernames.length
    ? profileUsernames
    : processState?.configuredUsernames
  return new Set((Array.isArray(configured) ? configured : [])
    .map((username) => String(username || '').trim().toLowerCase())
    .filter(Boolean))
}

function deliveryBotTimestamp(bot) {
  const timestamp = new Date(bot?.serverStatusAt || bot?.lastStatusAt || bot?.heartbeatAt || 0).getTime()
  return Number.isFinite(timestamp) ? timestamp : 0
}

function findDeliveryBot(processState = state.delivery?.process || {}) {
  const expectedUsernames = deliveryExpectedUsernames(processState)
  const deliveryBots = state.bots.filter(isDeliveryBot)
  ...
  return candidates
    ...
    .sort((left, right) => {
      ...
      return deliveryBotTimestamp(right) - deliveryBotTimestamp(left)
    })[0] || null
}

function titleDeliveryLocation(value) {
  const text = String(value || '').trim()
  if (!text) return 'Unknown'
  const normalized = text.toLowerCase()
  if (normalized.includes('lobby') || normalized.includes('spawn')) return 'Spawn Lobby'
  if (normalized === 'platform') return 'Printer Platform'
  return text
    .replace(/[-_]+/g, ' ')
    .replace(/\b[a-z]/g, (letter) => letter.toUpperCase())
}

function describeDeliveryBotPlace(bot) {
  if (!bot) {
    return {
      kind: 'offline',
      label: 'No Delivery Bot',
      detail: 'no delivery bot status'
    }
  }
  if (bot.online !== true) {
    return {
      kind: 'offline',
      label: 'Offline',
      detail: `${bot.botName || 'Delivery bot'} is offline`
    }
  }
  const delivery = bot.delivery && typeof bot.delivery === 'object' ? bot.delivery : null
  const place = delivery?.currentPlace && typeof delivery.currentPlace === 'object' ? delivery.currentPlace : null
  const kind = String(place?.kind || '').trim().toLowerCase()
  const detail = String(place?.detail || delivery?.location || bot.locationDetail || bot.location || '').trim()
  if (kind === 'delivery-station' || delivery?.stationLocation === 'at-station') {
    return {
      kind: 'delivery-station',
      label: 'Delivery Station',
      detail: detail || bot.botName || 'delivery bot'
    }
  }
  if (kind === 'printer-platform') {
    const label = String(place?.label || '').trim() || `${place?.hostLabel || place?.botName || 'Printer'} Platform`
    return {
  ...
  if (kind === 'lobby' || kind === 'spawn') {
    return {
      kind,
      label: 'Spawn Lobby',
      detail: detail || String(place?.label || '').trim() || bot.botName || 'delivery bot'
    }
  }
  const fallback = String(place?.label || bot.locationDetail || bot.location || delivery?.location || '').trim()
  return {
    kind: kind || (fallback ? 'away' : 'unknown'),
    label: titleDeliveryLocation(fallback),
    detail: detail || bot.botName || 'delivery bot'
  }
}

function deliveryPlaceStatusClass(place) {
  const kind = String(place?.kind || '').toLowerCase()
  if (kind === 'offline') return 'status-offline'
  if (kind === 'delivery-station' || kind === 'printer-platform') return 'status-online'
  return 'status-neutral'
}

function deliveryPlaceStatusText(place) {
  const kind = String(place?.kind || '').toLowerCase()
  if (kind === 'delivery-station') return 'station'
  if (kind === 'printer-platform') return 'platform'
  if (kind === 'lobby' || kind === 'spawn') return 'lobby'
  if (kind === 'offline') return 'offline'
  if (kind === 'unknown') return 'unknown'
  return 'away'
}

function deliveryBotControlsHtml(bot, processState = {}) {
  const botName = bot?.botName ? escapeHtml(bot.botName) : ''
  const online = bot?.online === true
  const admin = isAdmin()
  const botCommandDisabled = admin && botName && online ? '' : 'disabled'
  const startDisabled = admin && processState?.running !== true ? '' : 'disabled'
  const stopDisabled = admin && processState?.running === true ? '' : 'disabled'
  const adminDisabled = admin ? '' : 'disabled'
  const processLabel = processState?.running ? 'process running' : 'process stopped'
  const processClass = processState?.running ? 'status-online' : 'status-offline'
  return `
    <div class="delivery-process-tools">
      <span class="status-pill ${processClass}">${escapeHtml(processLabel)}</span>
    </div>
    <button class="accent-button small-button" type="button" data-action="delivery-process-start" data-permission-needed="admin" ${startDisabled}>Start</button>
    <button class="danger-button small-button" type="button" data-action="delivery-process-stop" data-permission-needed="admin" ${stopDisabled}>Stop</button>
    <button class="accent-button small-button" type="button" data-action="get-all-maps" data-permission-needed="admin" data-bot-name="${botName}" ${botCommandDisabled}>GetAllMaps</button>
    <button class="danger-button small-button" type="button" data-action="stop-mission" data-permission-needed="admin" data-bot-name="${botName}" ${botCommandDisabled}>Stop Mission</button>
    <button class="ghost-button small-button" type="button" data-action="disconnect-bot" data-permission-needed="admin" data-bot-name="${botName}" title="Disconnect from server (no auto-reconnect)" ${botCommandDisabled}>Disconnect</button>
    <button class="ghost-button small-button" type="button" data-action="delivery-process-reconnect" data-permission-needed="admin" title="Reconnect or restart delivery bot" ${adminDisabled}>Reconnect</button>
  `
}

function deliveryBotChatHtml(bot) {
  if (!bot?.botName) {
    return '<p class="chat-empty">Delivery chat will appear after the bot reports online.</p>'
  }
  const botName = escapeHtml(bot.botName)
  const disabled = isAdmin() && bot.online === true ? '' : 'disabled'
  return `
    <div class="chat-messages" id="chat-delivery-${botName}">
    ...
    <div class="chat-input-row">
      <input class="chat-input" type="text" placeholder="Send chat message..." maxlength="256"
        data-chat-bot="${botName}"
        data-permission-needed="admin" ${disabled} />
      <button class="accent-button small-button" type="button"
        data-action="chat-send"
        data-permission-needed="admin"
        data-bot-name="${botName}" ${disabled}>Send</button>
    </div>
  `
}
...
function renderBotCard(bot) {
  ...
  const showCoordinates = isAdmin()
    ...
    && bot.position
    && ['x', 'y', 'z'].every((axis) => Number.isFinite(Number(bot.position[axis])))
  const coordinatesText = showCoordinates
    ? ` | XYZ ${Number(bot.position.x).toFixed(1)}, ${Number(bot.position.y).toFixed(1)}, ${Number(bot.position.z).toFixed(1)}`
    : ''
  ...
  const verifyUrlField = verifyUrlText
    ? `<span class="verify-field">Open verification link: <a class="verify-url" href="${escapeHtml(verifyUrlText)}" target="_blank" rel="noopener noreferrer">${escapeHtml(verifyUrlText)}</a></span>`
    : ''
  const verifyCodeField = bot.verificationCode
    ? `<span class="verify-field">Legacy code: <strong class="verify-code">${escapeHtml(bot.verificationCode)}</strong></span>`
    : ''
  const verifyBanner = showVerify ? `
    <div class="verify-banner">
      <div class="verify-info">
        <span class="verify-label">Human Verification Required</span>
        <div class="verify-detail">
          <span class="verify-field">Username: <strong>${escapeHtml(bot.botName)}</strong></span>
          ${verifyUrlField}
          ${verifyCodeField}
          ${verifyIpField}
        </div>
      ...
      <div class="verify-actions">
        ...
        <button class="ghost-button small-button" type="button" data-action="verify-refresh" data-permission-needed="canControlBots" data-bot-name="${escapeHtml(bot.botName)}">New link</button>
  ...
  const deliveryMetricRows = isDeliveryBot ? `
      <div class="bot-metrics">
        <div class="metric metric-wide">Location<strong>${escapeHtml(deliveryPlace?.label || 'Unknown')}</strong></div>
        <div class="metric">Mission<strong>${escapeHtml(delivery?.missionStatus || 'idle')}</strong></div>
        <div class="metric">Stage<strong>${escapeHtml(delivery?.stage || statusDetail || 'idle')}</strong></div>
        <div class="metric">Target<strong>${escapeHtml(delivery?.targetBot || 'none')}</strong></div>
        <div class="metric">Waiting<strong>${escapeHtml(delivery?.targetsWaiting ?? 0)}</strong></div>
        <div class="metric">Collected<strong>${escapeHtml(delivery?.mapsCollected ?? 0)}</strong></div>
        <div class="metric">Deposited<strong>${escapeHtml(delivery?.mapsDeposited ?? 0)}</strong></div>
      </div>
      ${Array.isArray(delivery?.waitingTargets) && delivery.waitingTargets.length ? `
        <div class="bot-warnings">
          ${delivery.waitingTargets.slice(0, 4).map((target) => `
            <div class="bot-warning">
              <strong>${escapeHtml(target.botName || 'target')}</strong>
              <span>${escapeHtml(target.readiness || target.state || 'notready')}${target.location ? ` | ${escapeHtml(target.location)}` : ''}</span>
            </div>
          `).join('')}
        </div>` : ''}
    ` : ''
  const actionsHtml = isDeliveryBot ? `
        <button class="accent-button" type="button" data-action="start" data-permission-needed="canControlBots" data-bot-name="${escapeHtml(bot.botName)}">Start</button>
        <button class="danger-button" type="button" data-action="stop" data-permission-needed="canControlBots" data-bot-name="${escapeHtml(bot.botName)}">Stop</button>
        <button class="accent-button" type="button" data-action="get-all-maps" data-permission-needed="canControlBots" data-bot-name="${escapeHtml(bot.botName)}">GetAllMaps</button>
        <button class="danger-button small-button" type="button" data-action="stop-mission" data-permission-needed="canControlBots" data-bot-name="${escapeHtml(bot.botName)}">Stop Mission</button>
  ...
  return `
    <article class="bot-card${showVerify ? ' bot-card-verify' : ''}">
      ...
      <div class="bot-metrics">
        ...
        <div class="metric">Host<strong>${escapeHtml(bot.connectedHost || 'unknown')}</strong></div>
      ...
      <div class="bot-metrics">
        <div class="metric">${isDeliveryBot ? 'Target' : 'NBT'}<strong>${escapeHtml(bot.currentNbt || 'none')}</strong></div>
...
function renderNodeHostSelect(node) {
  const options = Array.isArray(state.hostOptions) && state.hostOptions.length
    ? state.hostOptions
    : ['alt.6b6t.org', 'alt3.6b6t.org', 'play.6b6t.org', 'alt2.6b6t.org', 'alt4.6b6t.org']
  const preferredHost = String(node.preferredHost || '').trim()
  const connectedHost = String(node.connectedHost || '').trim()
  const automaticLabel = connectedHost ? `Connected: ${connectedHost}` : 'Host: Automatic'
  const optionsHtml = [`<option value="default"${preferredHost ? '' : ' selected'}>${escapeHtml(automaticLabel)}</option>`]
    .concat(options.map((host) => {
      const selected = preferredHost === host
      const connectedSuffix = selected && connectedHost && connectedHost !== host ? `; connected ${connectedHost}` : ''
      return `<option value="${escapeHtml(host)}"${selected ? ' selected' : ''}>Host: ${escapeHtml(host)}${selected ? ` (pinned${escapeHtml(connectedSuffix)})` : ''}</option>`
    }))
    .join('')
  const connectedTitle = connectedHost ? ` Currently connected to ${connectedHost}.` : ''
  return `<select class="small-button node-host-select" data-action="set-node-host" data-permission-needed="canControlBots" data-host-label="${escapeHtml(node.hostLabel)}" title="Pin which 6b6t host this node connects to (Default = automatic lowest-ping selection).${escapeHtml(connectedTitle)}">${optionsHtml}</select>`
}

function renderBots() {
  const visibleBots = fleetBots()
  const visibleNodes = fleetNodes()
  const botsSignature = JSON.stringify({ bots: visibleBots, nodes: visibleNodes, dismissed: [...state.dismissedVerify] })
  if (state.renderCache.bots === botsSignature) return
  state.renderCache.bots = botsSignature

  if (!visibleNodes.length && !visibleBots.length) {
    elements.botsGrid.innerHTML = `
      <article class="empty-card">
        <h3>No bot data yet</h3>
        <p>Start at least one printer bot with dashboard integration enabled, then refresh this page.</p>
      </article>
    `
    return
  }
  ...
  const sortedNodes = visibleNodes.slice().sort(compareNodeLabels)
    ...
    return `
      <article id="${escapeHtml(nodeAnchorId(node.hostLabel, index))}" class="fleet-node-group">
        <div class="fleet-node-head">
          <div>
            <h3 class="bot-name" title="${escapeHtml(node.hostLabel)}">${escapeHtml(displayHostLabel(node.hostLabel))}</h3>
  ...
  const orphanBots = visibleBots.filter((bot) => !sortedNodes.some((node) => node.hostLabel === bot.hostLabel))
...
function renderBotInventory() {
  ...
  const sig = JSON.stringify({
    ...
    bots: visibleBots.map((bot) => `${bot.botName}:${bot.online}:${bot.hostLabel}`),
  ...
  if (!visibleBots.length) {
    elements.botInventoryGrid.innerHTML = `
      <article class="empty-card">
        ...
        <p>Printer bots will appear here after they report dashboard status.</p>
      </article>
    `
    return
  }
  ...
  elements.botInventoryGrid.innerHTML = visibleBots
    .slice()
        ...
        <article class="inventory-card">
          <div class="file-row">
            <div>
              ...
              <p class="file-meta">${escapeHtml(displayHostLabel(bot.hostLabel) || 'unknown-host')} | ${bot.online ? 'online' : 'offline'} | ${escapeHtml(stackCount)} stack(s), ${escapeHtml(totalCount)} item(s) | updated ${escapeHtml(formatTime(inventory.updatedAt))}</p>
            </div>
...
function renderFiles() {
  ...
  const sig = JSON.stringify({
    targetType: elements.uploadTargetType?.value || 'node',
    bots: visibleBots.map((b) => `${b.botName}:${b.hostLabel}:${b.online}`)
  })
  ...
  if (!byNode.size) {
    nodeOptions.push('<option value="" disabled>No printer bots available</option>')
  } else if (targetType === 'bot') {
    for (const [nodeLabel, bots] of byNode.entries()) {
      nodeOptions.push(`<optgroup label="${escapeHtml(displayHostLabel(nodeLabel))}">`)
    ...
    for (const [nodeLabel, bots] of byNode.entries()) {
      ...
      nodeOptions.push(`<option value="${escapeHtml(nodeLabel)}">${escapeHtml(displayHostLabel(nodeLabel))} (${onlineCount}/${bots.length} online)</option>`)
      nodeOptions.push('</optgroup>')
    }
  } else {
...
function renderNodes() {
  const visibleNodes = fleetNodes()
  const nodesSignature = JSON.stringify({ nodes: visibleNodes, nbtSearch: state.nbtSearch, canViewNodeFiles: hasPermission('canViewNodeFiles'), role: state.auth.role, isAdmin: isAdmin(), loaded: state.nodeInventoryLoaded, loading: state.nodeInventoryLoading })
  ...
  const totalFinishedMaps = visibleNodes.reduce((total, node) => {
  ...
  elements.nodesGrid.innerHTML = bulkFinishedMapActions + visibleNodes.map((node) => {
    ...
    return `
      <article class="node-card">
        <div class="file-row">
          <div>
            <strong title="${escapeHtml(node.hostLabel)}">${escapeHtml(displayHostLabel(node.hostLabel))}</strong>
...
function renderLogs() {
  const nodeLogGroups = fleetNodes()
    .map((node) => ({
    ...
    ? `
      <article class="file-item">
        <div>
          ...
          <p class="file-meta">All ${escapeHtml(groupedBotCount)} reporting bot(s) use hostLabel "${escapeHtml(displayHostLabel(nodeLogGroups[0].hostLabel))}", so logs are grouped under one node. Set a unique dashboard.hostLabel per machine/process if these should appear as separate nodes.</p>
        </div>
      </article>
    `
    : ''
  ...
  const nodeLogsMarkup = nodeLogGroups.map((group) => `
    <section class="log-download-group">
      <div class="log-download-group-head">
        <div>
          <strong>Node: ${escapeHtml(displayHostLabel(group.hostLabel))}</strong>
      ...
      ${group.items.length ? group.items.map((item) => {
        ...
        return `
          <article class="file-item compact-log-item">
            <div class="file-row">
              <div>
                <strong>${escapeHtml(item.fileName)}</strong>
                <p class="file-meta">Node: ${escapeHtml(displayHostLabel(group.hostLabel))} | ${escapeHtml(formatFileSize(item.sizeBytes))} | ${escapeHtml(formatTime(item.modifiedAt))}${escapeHtml(reporters)}</p>
              </div>
              <div class="file-actions">
        ...
        <article class="file-item">
          <div>
            ...
            <p class="file-meta">Node: ${escapeHtml(displayHostLabel(group.hostLabel))} | Wait for a bot heartbeat from this node.</p>
          </div>
        </article>
      `}
    </section>
  ...
  elements.configFilesList.innerHTML = configRows.map((configFile) => {
    ...
    const nodeActions = nodes.map((hostLabel) => `
      <button class="ghost-button small-button" type="button"
        ...
        title="Download ${escapeHtml(configFile.name)} from ${escapeHtml(displayHostLabel(hostLabel))}">Download ${escapeHtml(displayHostLabel(hostLabel))}</button>
    `).join('')
...
function selectedTeleportWhitelistFile() {
  const files = Array.isArray(state.teleportWhitelist.files) ? state.teleportWhitelist.files : []
  const selected = String(state.teleportWhitelist.selectedFile || '').trim()
...
function renderTeleportWhitelist() {
  ...
  if (elements.teleportWhitelistConfig) {
    elements.teleportWhitelistConfig.innerHTML = files.map((file) => (
  ...
  const whitelist = Array.isArray(selected.whitelist) ? selected.whitelist : []
  if (elements.teleportWhitelistStatus) {
    elements.teleportWhitelistStatus.textContent = `${selected.name} | ${whitelist.length} user${whitelist.length === 1 ? '' : 's'}`
  }
  ...
  if (elements.teleportWhitelistList) {
    ...
    if (!whitelist.length) {
      elements.teleportWhitelistList.innerHTML = `
        <article class="empty-card">
          <h3>No whitelisted users</h3>
          <p>Only admins can add users. Non-whitelisted teleport requests are ignored.</p>
        </article>`
      return
    }
    elements.teleportWhitelistList.innerHTML = whitelist.map((username) => `
      <article class="whitelist-user">
...
function deliveryReadinessClass(target) {
  const readiness = String(target?.readiness || '').toLowerCase()
  if (readiness === 'ready') return 'status-online'
  if (readiness === 'offline') return 'status-offline'
  return 'status-neutral'
}
...
function renderDelivery() {
  ...
  const canControl = hasPermission('canControlBots')
  const canAdmin = isAdmin()
  if (!canControl) {
    elements.deliveryPanel.classList.add('hidden')
    state.renderCache.delivery = ''
    return
  }
  elements.deliveryPanel.classList.remove('hidden')

  const station = delivery?.station || {}
  const targets = Array.isArray(delivery?.targets) ? delivery.targets : []
  const processState = delivery?.process && typeof delivery.process === 'object' ? delivery.process : {}
  const deliveryBot = findDeliveryBot(processState)
  const deliveryPlace = describeDeliveryBotPlace(deliveryBot)
  const deliveryStatusSig = deliveryBot
    ? {
        botName: deliveryBot.botName,
        online: deliveryBot.online,
        phase: deliveryBot.phase,
        statusDetail: deliveryBot.statusDetail,
        location: deliveryBot.location,
        locationDetail: deliveryBot.locationDetail,
        delivery: {
          currentPlace: deliveryBot.delivery?.currentPlace || null,
          location: deliveryBot.delivery?.location || null,
          stationLocation: deliveryBot.delivery?.stationLocation || null,
          missionStatus: deliveryBot.delivery?.missionStatus || null,
          stage: deliveryBot.delivery?.stage || null
        },
        recentChat: Array.isArray(deliveryBot.recentChat) ? deliveryBot.recentChat.slice(-12) : []
      }
    : null
  const sig = JSON.stringify({ station, targets, canControl, canAdmin, processState, deliveryStatusSig })
  if (state.renderCache.delivery === sig) return
  state.renderCache.delivery = sig

  if (elements.deliveryLocationSummary) {
    elements.deliveryLocationSummary.innerHTML = `
      <div class="delivery-location-copy">
        <span class="delivery-location-kicker">Current Location</span>
        <strong>${escapeHtml(deliveryPlace.label)}</strong>
        <span>${escapeHtml(deliveryBot?.botName ? `${deliveryBot.botName} | ${deliveryPlace.detail}` : deliveryPlace.detail)}</span>
      </div>
      <div class="delivery-control-panel">
        <div class="delivery-location-pills">
          <span class="status-pill ${deliveryBot?.online ? 'status-online' : 'status-offline'}">${deliveryBot?.online ? 'online' : 'offline'}</span>
          <span class="phase-pill ${phaseClass(deliveryBot?.statusDetail || deliveryBot?.phase || 'unknown')}">${escapeHtml(deliveryBot?.statusDetail || deliveryBot?.phase || 'unknown')}</span>
          <span class="status-pill ${deliveryPlaceStatusClass(deliveryPlace)}">${escapeHtml(deliveryPlaceStatusText(deliveryPlace))}</span>
        </div>
        <div class="delivery-bot-actions">
          ${deliveryBotControlsHtml(deliveryBot, processState)}
        </div>
      </div>
    `
  }
  if (elements.deliveryChatPanel) {
    elements.deliveryChatPanel.innerHTML = deliveryBotChatHtml(deliveryBot)
  }
  if (elements.deliveryStationStatus) {
    const ready = targets.filter((target) => target.readiness === 'ready').length
    const offline = targets.filter((target) => target.readiness === 'offline').length
    const notReady = targets.length - ready - offline
    elements.deliveryStationStatus.textContent = `${targets.length} printer target(s): ${ready} ready, ${notReady} not ready, ${offline} offline`
  }

  if (!elements.deliveryTargetsList) return
  if (!targets.length) {
    elements.deliveryTargetsList.innerHTML = `
      <article class="empty-card">
        <h3>No delivery targets</h3>
        <p>Printer bots will appear here after they report ready status.</p>
      </article>`
    return
  }
  ...
  elements.deliveryTargetsList.innerHTML = `
    <div class="resource-metrics-table-wrap">
      <table class="resource-metrics-table delivery-target-table">
        <thead>
          <tr>
            <th scope="col">Bot</th>
        ...
        <tbody>
          ${targets.map((target) => {
            ...
            const detail = target.readinessDetail || target.lastStatusAt || ''
            return `
              <tr>
                <th scope="row">
                  <strong>${escapeHtml(target.botName || 'unknown')}</strong>
                  <span>${escapeHtml(source)}${target.hostLabel ? ` | ${escapeHtml(displayHostLabel(target.hostLabel))}` : ''}</span>
                </th>
                <td>
                  <span class="status-pill ${deliveryReadinessClass(target)}">${escapeHtml(target.readiness || 'unknown')}</span>
                  ${detail ? `<span class="metric-subtle">${escapeHtml(detail)}</span>` : ''}
                </td>
                <td>${escapeHtml(target.locationDetail || target.location || 'n/a')}</td>
                <td>${escapeHtml(target.order ?? 100)}</td>
                <td>
                  <button class="ghost-button small-button" type="button"
                    data-action="${enabled ? 'delivery-target-disable' : 'delivery-target-enable'}"
                    data-permission-needed="admin"
                    data-bot-name="${escapeHtml(target.botName || '')}"
                    data-target-order="${escapeHtml(target.order ?? 100)}" ${canAdmin ? '' : 'disabled'}>${enabled ? 'Disable' : 'Enable'}</button>
                  ${source === 'manual' ? `<button class="danger-button small-button" type="button"
                    data-action="delivery-target-delete"
                    data-permission-needed="admin"
                    data-bot-name="${escapeHtml(target.botName || '')}" ${canAdmin ? '' : 'disabled'}>Delete</button>` : ''}
                </td>
              </tr>
            `
          }).join('')}
...
async function refreshTeleportWhitelist() {
  ...
  state.teleportWhitelist.selectedFile = previous
  state.renderCache.teleportWhitelist = ''
  renderTeleportWhitelist()
}

async function onAddTeleportWhitelistUser() {
...
async function onRemoveTeleportWhitelistUser(username) {
  if (!hasPermission('admin')) return
  const file = selectedTeleportWhitelistFile()
...
async function refreshData(options = {}) {
  ...
  try {
    ...
    state.hostOptions = Array.isArray(snapshot.hostOptions) ? snapshot.hostOptions : state.hostOptions
    state.delivery = snapshot.delivery && typeof snapshot.delivery === 'object' ? snapshot.delivery : null
...
async function onSendChat(botName, message) {
  const bot = state.bots.find((entry) => String(entry?.botName || '').toLowerCase() === String(botName || '').toLowerCase())
  const allowed = isDeliveryBot(bot) ? isAdmin() : hasPermission('canControlBots')
  if (!allowed) {
    pushEvent('warn', isDeliveryBot(bot) ? 'Login as an admin before sending delivery chat.' : 'Login as an operator before sending chat.')
    return
  }
...
async function onStartDeliveryProcess() {
  if (!isAdmin()) {
    pushEvent('warn', 'Login as an admin before starting the delivery bot.')
    return
  }
  const result = await submitJson('/api/dashboard/delivery/process/start', {})
  pushEvent('info', result.started
    ? `Started delivery bot on ${result.process?.connection || 'auto-detected profile'}`
    : `Delivery bot process is already running on ${result.process?.connection || 'auto-detected profile'}`)
  await refreshData()
}

async function onStopDeliveryProcess() {
  if (!isAdmin()) {
    pushEvent('warn', 'Login as an admin before stopping the delivery bot.')
    return
  }
  const result = await submitJson('/api/dashboard/delivery/process/stop', {})
  pushEvent(result.stopped ? 'warn' : 'info', result.stopped ? 'Stopped delivery bot process' : 'Delivery bot process was not running')
  await refreshData()
}

async function onReconnectDeliveryProcess() {
  if (!isAdmin()) {
    pushEvent('warn', 'Login as an admin before reconnecting the delivery bot.')
    return
  }
  const result = await submitJson('/api/dashboard/delivery/process/reconnect', {})
  if (result.mode === 'command') {
    pushEvent('info', 'Queued reconnect for the online delivery bot')
  } else {
    pushEvent('info', `${result.mode === 'restart' ? 'Restarted' : 'Started'} delivery bot on ${result.process?.connection || 'auto-detected profile'}`)
  }
  await refreshData()
}
...
async function onDeleteAllFinishedMaps() {
  ...
  const totalFinishedMaps = fleetNodes().reduce((total, node) => {
    return total + (Array.isArray(node.finishedMapFiles) ? node.finishedMapFiles.length : 0)
  }, 0)
...
function deliveryPointFromInputs(xEl, yEl, zEl) {
  const values = [xEl?.value, yEl?.value, zEl?.value].map((value) => String(value ?? '').trim())
...
async function onSaveDeliveryStation(event) {
  event.preventDefault()
  if (!isAdmin()) {
    pushEvent('warn', 'Login as an admin before changing the delivery station target.')
    return
  }
  const anchor = deliveryPointFromInputs(elements.deliveryStationX, elements.deliveryStationY, elements.deliveryStationZ)
  if (!anchor) {
    pushEvent('warn', 'Delivery station anchor needs numeric x/y/z values.')
    return
  }
  const payload = { anchor }
  const result = await submitJson('/api/dashboard/delivery/station', payload)
  state.delivery = { ...(state.delivery || {}), station: result.station || payload }
  elements.deliveryStationForm?.reset()
  state.renderCache.delivery = ''
  renderDelivery()
  pushEvent('info', 'Saved new delivery station target anchor')
  await refreshData()
}

async function onSaveDeliveryManualTarget(event) {
  event.preventDefault()
  if (!isAdmin()) {
    pushEvent('warn', 'Login as an admin before editing delivery targets.')
    return
  }
  const botName = elements.deliveryManualBotName?.value?.trim() || ''
  const anchor = deliveryPointFromInputs(elements.deliveryManualX, elements.deliveryManualY, elements.deliveryManualZ)
  if (!botName || !anchor) {
    pushEvent('warn', 'Manual target needs a bot name and numeric anchor.')
    return
  }
  await submitJson('/api/dashboard/delivery/targets', {
    botName,
    anchor,
    enabled: true,
    order: Number(elements.deliveryManualOrder?.value || 100)
  })
  elements.deliveryManualForm?.reset()
  pushEvent('info', `Saved delivery target ${botName}`)
  await refreshData()
}

async function onToggleDeliveryTarget(botName, enabled, order = 100) {
  if (!isAdmin()) {
    pushEvent('warn', 'Login as an admin before editing delivery targets.')
    return
  }
  ...
  pushEvent('info', `${enabled ? 'Enabled' : 'Disabled'} delivery target ${botName}`)
  await refreshData()
}

async function onDeleteDeliveryTarget(botName) {
  if (!isAdmin()) {
    pushEvent('warn', 'Login as an admin before editing delivery targets.')
    return
  }
  await submitJson(`/api/dashboard/delivery/targets/${encodeURIComponent(botName)}/delete`, {})
  pushEvent('warn', `Deleted delivery target ${botName}`)
  await refreshData()
}
...
async function onFleetAction(action, botName = null) {
  ...
  } else if (action === 'home-all') {
    const result = await submitJson('/api/dashboard/commands/home-all', {})
    const count = Array.isArray(result?.items) ? result.items.length : 0
    pushEvent('info', `Queued /home platform for ${count} known bot(s); unready bots will wait`)
...
document.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (button.dataset.action === 'delete-node-file') {
    ...
    } else if (button.dataset.action === 'remove-teleport-whitelist-user') {
      await onRemoveTeleportWhitelistUser(button.dataset.username || '')
      ...
      renderBots()
    } else if (button.dataset.action === 'delivery-process-start') {
      await onStartDeliveryProcess()
    } else if (button.dataset.action === 'delivery-process-stop') {
      await onStopDeliveryProcess()
    } else if (button.dataset.action === 'delivery-process-reconnect') {
      await onReconnectDeliveryProcess()
    ...
    } else if (button.dataset.action === 'get-all-maps') {
      await submitJson(`/api/dashboard/bots/${encodeURIComponent(button.dataset.botName || '')}/commands/get-all-maps`, {})
      pushEvent('info', `Queued GetAllMaps for ${button.dataset.botName || 'delivery bot'}`)
      await refreshData()
    } else if (button.dataset.action === 'stop-mission') {
      await submitJson(`/api/dashboard/bots/${encodeURIComponent(button.dataset.botName || '')}/commands/stop-mission`, {})
      pushEvent('warn', `Queued delivery mission stop for ${button.dataset.botName || 'delivery bot'}`)
      await refreshData()
    } else if (button.dataset.action === 'delivery-target-enable') {
      await onToggleDeliveryTarget(button.dataset.botName || '', true, button.dataset.targetOrder || 100)
    } else if (button.dataset.action === 'delivery-target-disable') {
      await onToggleDeliveryTarget(button.dataset.botName || '', false, button.dataset.targetOrder || 100)
    } else if (button.dataset.action === 'delivery-target-delete') {
      await onDeleteDeliveryTarget(button.dataset.botName || '')
...
elements.homeAllButton.addEventListener('click', async () => {
  try {
    elements.homeAllButton.disabled = true
    await onFleetAction('home-all')
  } catch (error) {
    pushEvent('error', error.message)
  } finally {
    elements.homeAllButton.disabled = false
  }
})
...
if (elements.teleportWhitelistForm) {
  elements.teleportWhitelistForm.addEventListener('submit', (event) => {
    event.preventDefault()
    void onAddTeleportWhitelistUser().catch((error) => pushEvent('error', error.message))
  })
}

if (elements.deliveryStationForm) {
  elements.deliveryStationForm.addEventListener('submit', (event) => {
    void onSaveDeliveryStation(event).catch((error) => pushEvent('error', error.message))
  })
}

if (elements.deliveryManualForm) {
  elements.deliveryManualForm.addEventListener('submit', (event) => {
    void onSaveDeliveryManualTarget(event).catch((error) => pushEvent('error', error.message))
  })
}
...
document.addEventListener('change', async (event) => {
  const select = event.target.closest('select[data-action="set-node-host"]')
  if (!select) return
  if (!hasPermission('canControlBots')) {
    pushEvent('warn', 'Login with bot-control access before changing the connection host.')
    return
  }
  const hostLabel = select.dataset.hostLabel || ''
  const host = select.value || 'default'
  if (!hostLabel) return
  try {
    select.disabled = true
    await submitJson(`/api/dashboard/nodes/${encodeURIComponent(hostLabel)}/preferred-host`, { host })
    pushEvent('info', host === 'default'
      ? `Cleared host pin for node ${hostLabel}; using automatic selection.`
      : `Pinned node ${hostLabel} to ${host}. Applies on the next (re)connect.`)
    await refreshData()
  } catch (error) {
    pushEvent('error', error.message)
  } finally {
